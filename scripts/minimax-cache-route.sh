...
# Verified against a copy on port 8099: patched shim read 2,944 cached tokens
# where the live shim read 0. The auth swap below is required — without it the
# proxy returns 401.
...
case "${1:-status}" in
  ...
  apply)
    ...
    KEY=$(/usr/bin/python3 -c "import re;t=open('$HOME/.config/go-llm-proxy/minimax-codex.yaml').read();print(re.search(r'keys:\\s*\\n\\s*-\\s*key:\\s*[\"\x27]?([^\"\x27\\s]+)',t).group(1))" 2>/dev/null)
    [ -n "$KEY" ] || fail "could not read proxy gate key — the swap would 401"
...
new="""      // MiniMax goes to the proxy, which honours cache_control; Bifrost strips
      ...
      if (isMM) {
        target = new URL(process.env.MINIMAX_UPSTREAM || 'http://127.0.0.1:8789');
        // The proxy is a separate service with its own gate credential; the
        // Bifrost virtual key means nothing to it. Verified 2026-08-04: without
        // this swap the patched shim returns 401 invalid API key.
        const proxyKey = process.env.MINIMAX_PROXY_KEY;
        if (proxyKey) { headers.authorization = `Bearer ${proxyKey}`; delete headers['x-api-key']; }
      }"""
