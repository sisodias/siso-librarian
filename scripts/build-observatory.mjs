...
const derivations = {
  ...
  'god_questions.coverage.with_testable_contract': { source: join(registry, 'works'), kind: 'json-predicate-count',
    query: 'frontier-question-*.json research_contract.success_criteria+ research_contract.falsifiers+ research_contract.watch_triggers+' },
  'release_integrity.works_referenced': { source: join(registry, 'releases'), kind: 'json-join-count', query: '*.json work_id' },
  'release_integrity.orphaned_releases': { source: join(registry, 'releases'), kind: 'json-join-count',
...
<section><h2>God Questions (registry)</h2><p>Every registered question and the state of its research contract. A question with zero falsifiers cannot be contradicted by evidence.</p><ul>${snapshot.god_questions.questions.map(q=>`<li><b>${q.id}</b> ${String(q.title).replace(/[&<>]/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[ch]))} — <i>${q.state || 'no state'}</i> · criteria ${q.success_criteria} · falsifiers ${q.falsifiers} · triggers ${q.watch_triggers} · gaps ${q.evidence_gaps}</li>`).join('')}</ul></section>
${(snapshot.awaiting_decision.items || []).length ? `<section><h2>Decisions waiting on you</h2><p>Each was re-verified 2026-08-04 against current state. The full text with evidence is in <code>proposals/2026-08-04-decisions-awaiting-shaan.md</code>.</p><ul>${snapshot.awaiting_decision.items.map(d=>`<li>${String(d).replace(/[&<>]/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[ch]))}</li>`).join('')}</ul></section>` : ''}
